...
import { ArrowLeft, ExternalLink, Youtube as YT } from "lucide-react";
...
const Body = ({ id }) => {
  ...
  if (!v) {
    return (
      <>
        <Nav />
        <main className="container-x" style={{ paddingTop: 80, paddingBottom: 96, textAlign: "center" }}>
          <p style={{ color: "var(--fg-dim)", fontSize: 14 }}>video not found · <a href="/" className="link-u" style={{ color: "var(--accent)" }}>cd ~</a></p>
        </main>
      </>
    );
  }
  ...
  return (
    <>
      ...
      <main className="container-x" style={{ paddingTop: 32, paddingBottom: 96, maxWidth: 1100 }}>
        <a href="/youtube" data-testid="video-back" className="link-u" style={{ display: "inline-flex", alignItems: "center", gap: 8, color: "var(--fg-dim)", fontSize: 13, textDecoration: "none", marginBottom: 24 }}>
        ...
        <div className="reveal" style={{ animationDelay: "0.05s" }}>
          ...
          <h1 data-testid="video-title" style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "clamp(24px, 3.4vw, 36px)", fontWeight: 700, lineHeight: 1.15, letterSpacing: "-0.01em", color: "var(--fg)", margin: "0 0 10px 0" }}>
        ...
        <div className="reveal win" style={{ animationDelay: "0.12s", overflow: "hidden", marginBottom: 28 }} data-testid="video-player">
        ...
        <div className="reveal" style={{ animationDelay: "0.2s", marginBottom: 28 }}>
          <TerminalWindow title="cat description.md" path={`~/videos/${v.id}`}>
            ...
            <div style={{ marginTop: 22, display: "flex", flexWrap: "wrap", gap: 8 }}>
              {v.tags.map((t_) => <span key={t_} className="tag">#{t_.toLowerCase()}</span>)}
            </div>
        ...
        <div className="reveal" style={{ animationDelay: "0.28s", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16 }}>
          <a href={`/video/${prev.id}`} data-testid="video-prev" className="card-term" style={{ textDecoration: "none", display: "block" }}>
            <div style={{ color: "var(--fg-mute)", fontSize: 11, marginBottom: 6, fontFamily: "'JetBrains Mono', monospace" }}>◄ prev · ./{prev.id}</div>
            <div style={{ color: "var(--fg)", fontSize: 14, lineHeight: 1.5 }}>{prev.title}</div>
          </a>
          <a href={`/video/${next.id}`} data-testid="video-next" className="card-term" style={{ textDecoration: "none", display: "block", textAlign: "right" }}>
            <div style={{ color: "var(--fg-mute)", fontSize: 11, marginBottom: 6, fontFamily: "'JetBrains Mono', monospace" }}>next ► · ./{next.id}</div>
            <div style={{ color: "var(--fg)", fontSize: 14, lineHeight: 1.5 }}>{next.title}</div>
          </a>
        </div>
      </main>
    </>
...
export default function VideoIsland({ id }) {
  return <LanguageProvider><Body id={id} /></LanguageProvider>;
}
